...
export default function Reports() {
  ...
  const fetchReports = async (token) => {
    try {
      ...
      const endDate = new Date(now.getFullYear() + 1, 5, 30); // June 30th next year
              ...
              const response = await fetch(`https://hbg-budget-app-7.onrender.com/api/reports/financial-summary?startDate=${startDate.toISOString().split('T')[0]}&endDate=${endDate.toISOString().split('T')[0]}`, {
        headers: {
  ...
  const downloadReport = async (type) => {
    try {
      const token = localStorage.getItem('token');
              const response = await fetch(`https://hbg-budget-app-7.onrender.com/api/reports/export/transactions?format=${type}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
